...
<div style="width: 80%; margin: auto;">
    ...
    <p>
        ...
        <?php if(!isset($group)) : ?>
        <form class="form-inline form-import" method="POST" action="<?= action('Vsch\TranslationManager\Controller@postImport') ?>" data-remote="true" role="form">
        ...
        <form class="form-inline form-find" method="POST" action="<?= action('Vsch\TranslationManager\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to scan you app folder? All found translation keys will be added to the database.">
            <button type="submit" class="btn btn-info" data-disable-with="Searching.." >Find translations in files</button>
        </form>
        <?php endif; ?>
        <?php if(isset($group)) : ?>
            <form class="form-inline form-publish" method="POST" action="<?= action('Vsch\TranslationManager\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="Are you sure you want to publish the translations group '<?= $group ?>? This will overwrite existing language files.">
    ...
    <table class="table">
        ...
        <?php foreach($translations as $key => $translation): ?>
            <tr id="<?= $key ?>">
                ...
                <?php if($deleteEnabled): ?>
                    <td>
                        <a href="<?= action('Vsch\TranslationManager\Controller@postDelete', [$group, $key]) ?>" class="delete-key" data-method="POST" data-remote="true" data-confirm="Are you sure you want to delete the translations for '<?= $key ?>?"><span class="glyphicon glyphicon-trash"></span></a>
                    </td>
                <?php endif; ?>
            </tr>
        <?php endforeach; ?>
