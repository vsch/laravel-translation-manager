...
<table class="table">
    <thead>
    <tr>
        <th width="15%"><?= trans('translations.group') ?></th>
        <th width="20%"><?= trans('translations.key') ?></th>
        <th width=" 5%"><?= trans('translations.locale') ?></th>
        <th width="60%"><?= trans('translations.translation') ?></th>
    </tr>
    </thead>
    <tbody>
        ...
        <tr>
            ...
            <td>
                <a href="#edit" class="editable status-<?= $t ? $t->status : 0 ?> locale-<?= $t->locale ?>" data-locale="<?= $t->locale ?>"
                    ...
                    data-title="<?=trans('translations.enter-translation')?>"><?= $t ? htmlentities($t->value, ENT_QUOTES, 'UTF-8', false) : '' ?></a> <?= !$t ? '' : ($t->saved_value === $t->value ? '' : ' [' . \Barryvdh\TranslationManager\Controller::mb_renderDiffHtml($t->saved_value, $t->value) . ']') ?>
            </td>
        </tr>
    <?php endforeach; ?>
    </tbody>
